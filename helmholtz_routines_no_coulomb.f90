      ...
      SUBROUTINE read_helmholtz

!************************************************************
!                                                           *
!     Calls the Helmholtz-EOS for the first time, thereby   *
!     reading in the tables; SKR 21.1.2003                  *
!                                                           *
!************************************************************
!
      IMPLICIT NONE
      ...
      double precision aion(ionmax),zion(ionmax),abar,zbar


!
!--just some typical values as dummies
!
!..hydrogen
      xmass(1) = 0.d0
      aion(1)  = 1.0d0
...
!..set the input vector (cgs-units)
!     old temperature as guess
      temp_row(1) = 1.d8
      den_row(1)  = 1.d6
      abar_row(1) = abar
      zbar_row(1) = zbar
...
!..for the coulomb corrections
      double precision dsdd,dsda,lami,inv_lami,lamida,lamidd, &
                       ...
                       third,esqu
      parameter        (third = 1.0d0/3.0d0,esqu  = qe * qe)
...
!..open the table
!       open(unit=2,file=
!     & '/home/srosswog/code_development/EOS/helmholtz/helm_table.dat',
!     &  status='old')       
!     ALTIX
      OPEN(unit=2,file='helm_table.dat', &
           status='old')
...
!..ion section:
       ...
       x       = avo*kerg/abar
       dsionda = (dpionda*deni + deionda)*tempi + kergavo*ytot1*ytot1* (2.5d0 - y)
       dsiondz = 0.0d0
